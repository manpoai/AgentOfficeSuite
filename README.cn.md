...
AgentOffice 是个本地服务，默认监听 `http://localhost:3000`（Shell）和 `http://localhost:4000`（Gateway）。推荐的部署方式是 **AgentOffice 和 Agent 跑在同一台机器上**，Agent 会通过 `http://localhost:4000` 直接连进来，零配置。

如果你想从另一台设备访问 AgentOffice，或者让 Agent 跑在另一台机器上，看下面 [自定义对外 URL](#自定义对外-url) 一节。

### 2. 日常使用（推荐）

日常使用建议把 AgentOffice 全局安装，这样可以用后台模式、状态查询和一键升级：
...
```bash
agentoffice-main start -d   # 后台启动
agentoffice-main status     # 查看状态、版本、健康
agentoffice-main stop       # 停止
agentoffice-main restart    # 重启
agentoffice-main logs -f    # 实时日志
agentoffice-main update     # 下载最新 runtime 并重启
agentoffice-main version    # 查看 bootstrap 和 runtime 版本
```

`npx agentoffice-main` 和全局安装版共用同一个数据目录（`~/.agentoffice/`），可以随时来回切换不会丢数据。Bootstrap 本身用 `npm install -g agentoffice-main@latest` 升级，runtime 用 `agentoffice-main update` 单独升级，两者刻意解耦。

---

## 自定义对外 URL

默认的同机器部署不需要任何配置。这一节只在你想换个地址访问 AgentOffice 的时候才会用到——比如你把 AgentOffice 跑在一台机器上，想让另一台机器上的 Agent 来连。

### 第 1 步 —— 把 AgentOffice 暴露到你选定的地址
...
| 步骤 | 阶段 | 说明 |
|---|---|---|
| 第 1 步 | Onboard | 在 AgentOffice 里复制 onboarding prompt，发送给 Agent，Agent 会发起注册申请 |
| 第 2 步 | 激活 | 在 AgentOffice 中审批 Agent 的注册申请 |
| 第 3 步 | 开始协作 | 你可以在原聊天平台给 Agent 下达任务，也可以直接在 AgentOffice 的评论中通过 `@agent` 发起协作 |

如果你之后把 AgentOffice 换到新地址，参考上面的 [自定义对外 URL](#自定义对外-url) 一节让 Agent 切过去。
...
- Agent 可通过 onboarding prompt 自注册，并使用已配置的 AgentOffice 公网地址接入
- 注册申请需要经过审批后才能激活
