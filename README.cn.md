...
[English](./README.md) | <u>中文</u> | [日本語](./README.ja.md) | [X](https://x.com/manpo_wang)
...
| 画布 | 无限画布编辑器，支持多页自由版面设计 | 形状、文本、图片、SVG 路径、画框（页面）；组合/取消组合；矢量编辑；布尔运算；按帧导出 PNG/SVG |
| 视频 | 基于时间线的动态图形编辑器 | 场景式时间线；关键帧动画与缓动曲线；形状/文本/图片元素；通过 ffmpeg.wasm 导出 MP4/WebM |
...
在任何 Agent 编辑发生之前——无论是文档、数据库、演示文稿、流程图、画布还是视频——AOSE 都会自动创建一份版本快照。每次变更都归因到执行它的 Agent，并附有时间戳。
...
| **画布** | 无限画布编辑器，支持多页面自由版面设计 | 矩形、圆形、文本、图片、SVG 矢量路径、画框（页面）；组合/取消组合/布尔运算；矢量点编辑；钢笔工具；按帧导出 PNG/SVG；元素和页面级评论锚定 |
| **视频** | 基于时间线的动态图形编辑器，支持关键帧动画 | 场景式时间线，每元素独立轨道；关键帧标记，8 种缓动预设；形状/文本/图片/线条元素；属性动画（位置、大小、旋转、透明度、颜色）；通过 ffmpeg.wasm 导出 MP4/WebM |
| **评论** | 统一评论系统，所有内容类型共享同一套评论基础设施 | 精准锚定到文档选区、图片、表格行、幻灯片元素、流程图节点或连线、画布元素或页面、视频元素；`@agent` 实时通知；事件包含锚点上下文和内容片段 |
| **历史版本** | 版本历史和恢复能力，为 Agent 参与的工作流提供安全网 | Agent 编辑前自动创建版本快照；任意版本一键恢复；文档/数据表/演示文稿/流程图/画布/视频统一管理 |
